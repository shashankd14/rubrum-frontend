                                ...
                                if (field.type === 'calendar') {
                                        inputEle = (
                                            <DatePicker
                                                ...
                                                onChange={(date, dateString) => setDateValue(date, dateString, index)}
                                            />
                                        )
                                }
                                        ...
                                        inputEle = (
                                            <Radio.Group value={field.value}>
                                                {options.map((item, index) => (
                                                    <Radio value={camelCase(item)} key={index} onChange={(e) => setFeildValue(e, index)}>{item}</Radio>
                                                ))}
                                            </Radio.Group>
                                        );
                                } else if (field.type === 'textArea') {
                                        inputEle = (
                                            <Input.TextArea type={field.type} value={field.value} onChange={(e) => setFeildValue(e, index)}>
                                            </Input.TextArea>
                                        )
                                } else if (field.type === 'number') {
                                    inputEle = (
                                        <Input type={field.type} max={field.max} min={field.min} value={field.value} onChange={(e) => setFeildValue(e, index)}>
                                        </Input>
                                    );
                                } else {
                                    inputEle = (
                                        <Input type={field.type} value={field.value} onChange={(e) => setFeildValue(e, index)}>
                                        </Input>
                                    );
                                }
                                return (
