...
const Party = props => {
  ...
  const [showAmtDcPdfFlg, setShowAmtDcPdfFlg] = useState(
    props.party?.showAmtDcPdfFlg === 'Y',
  ); // Default value
  ...
  const onEdit = (record, e) => {
    ...
    let classificationObj =
      record.packetClassificationTags.length === 0
        ? [{ tagId: 1 }]
        : record.packetClassificationTags;
  ...
  useEffect(() => {
    ...
    if (searchValue) {
      const filteredData = party?.partyList?.filter(party => {
        if (
          party.nPartyId?.toString() === searchValue ||
  ...
  const {
    dailyReportsList: initialDailyReportsList,
    monthlyReportsList: initialMonthlyReportsList,
    ...otherProps
  } = props.party.party;
  ...
  return (
    <div>
      <h1>
        <IntlMessages id="sidebar.company.partyList" />
      </h1>
      <Card>
        <div className="gx-flex-row gx-flex-1">
          ...
          <div className="gx-flex-row gx-w-50">
            ...
            <SearchBox
              styleName="gx-flex-1"
              placeholder="Search for party id or party name..."
              value={searchValue}
          ...
          <Card className="gx-card">
            <Row>
              <Col span={24}>
                <Card>
                  <p>
                    <strong>Party Name :</strong> {viewPartyDate?.partyName}
                  </p>
                  {viewPartyDate?.partyNickname && (
                    <p>
                      <strong>Party Nickname :</strong>{' '}
                      {viewPartyDate?.partyNickname}
                    </p>
                  )}
                  <p>
                    <strong>Phone Number :</strong> {viewPartyDate?.phone1}
                  </p>
                  {viewPartyDate?.phone2 && (
                    <p>
                      <strong>Alternate phone number 1 :</strong>{' '}
                      {viewPartyDate?.phone2}
                    </p>
                  )}
                  {viewPartyDate?.phone3 && (
                    <p>
                      <strong>Alternate phone number 2:</strong>{' '}
                      {viewPartyDate?.phone3}
                    </p>
                  )}
                  <p>
                    <strong>E-mail :</strong> {viewPartyDate?.email1}
                  </p>
                  {viewPartyDate?.email2 && (
                    <p>
                      <strong>Alternate E-mail 1:</strong>{' '}
                      {viewPartyDate?.email2}
                    </p>
                  )}
                  {viewPartyDate?.email3 && (
                    <p>
                      <strong>Alternate E-mail 2:</strong>{' '}
                      {viewPartyDate?.email3}
                    </p>
                  )}
                  {viewPartyDate?.contactName && (
                    <p>
                      <strong>Contact Name :</strong>{' '}
                      {viewPartyDate?.contactName}
                    </p>
                  )}
                  {viewPartyDate?.contactNumber && (
                    <p>
                      <strong>Contact Number :</strong>{' '}
                      {viewPartyDate?.contactNumber}
                    </p>
                  )}
                  {viewPartyDate?.tanNumber && (
                    <p>
                      <strong>TAN Number :</strong> {viewPartyDate?.tanNumber}
                    </p>
                  )}
                  {viewPartyDate?.panNumber && (
                    <p>
                      <strong>PAN Number :</strong> {viewPartyDate?.panNumber}
                    </p>
                  )}
                  {viewPartyDate?.gstNumber && (
                    <p>
                      <strong>GST Number :</strong> {viewPartyDate?.gstNumber}
                    </p>
                  )}
                  {viewPartyDate?.address1 && (
                    <>
                      <p>
                        ...
                        {viewPartyDate?.address1?.details}
                      </p>
                      <p>
                        <strong>City :</strong> {viewPartyDate?.address1?.city}
                      </p>
                      <p>
                        <strong>State :</strong>{' '}
                        {viewPartyDate?.address1?.state}
                      </p>
                      <p>
                        <strong>Pincode :</strong>{' '}
                        {viewPartyDate?.address1?.pincode}
                      </p>
                    </>
                  )}
                  {viewPartyDate?.packetClassificationTags && (
                    <p>
                      <strong>Tags:</strong>
                      {viewPartyDate?.packetClassificationTags?.map(
                        item => item.tagName,
                      )}
                    </p>
                  )}
                  {viewPartyDate?.endUserTags && (
                    <p>
                      <strong>EndUser Tags:</strong>
                      {viewPartyDate?.endUserTags?.map(item => item.tagName)}
                    </p>
                  )}
                  {viewPartyDate?.showAmtDcPdfFlg && (
                    <p>
                      <strong>Include Rates in Delivery Challan :</strong>{' '}
                      {viewPartyDate?.showAmtDcPdfFlg || 'N'}
                    </p>
                  )}
                  {props.partyId.dailyReportsList && (
                    <p>
                      <strong>Daily Reports List :</strong>{' '}
                      {props.partyId?.dailyReportsList}
                    </p>
                  )}
                  {props.partyId?.monthlyReportsList && (
                    <p>
                      <strong>Monthly Reports List :</strong>{' '}
                      {props.partyId?.monthlyReportsList}
                    </p>
                  )}
                </Card>
        ...
        <Modal
          ...
          onOk={e => {
            if (editParty) {
              ...
              setDailyReportsList(
                initialDailyReportsList
                  ? initialDailyReportsList.split(',')
                  : [],
              );
              setMonthlyReportsList(
                initialMonthlyReportsList
                  ? initialMonthlyReportsList.split(',')
                  : [],
              );
              e.preventDefault();
                ...
                <Form {...formItemLayout} className="gx-pt-4">
                  <Form.Item label="Party/Customer Name">
                    {getFieldDecorator('partyName', {
                      rules: [
                        { required: true, message: 'Please input Party name!' },
                      ],
                    })(<Input id="partyName" />)}
                  </Form.Item>
                  ...
                  {phoneKeys.map((k, index) => {
                    ...
                    const phone = party?.phone3
                      ? [party?.phone1, party?.phone2, party?.phone3]
                      : party?.phone2
                      ...
                      >
                        {getFieldDecorator(`phone[${index}]`, {
                          ...
                          rules: [
                            {
                              required: req,
                              message: `Please input your ${index ? `Alternate Phone Number ${index}!` : 'Phone Number!'}`,
                            },
                          ],
                        })(
                          <Input
                        ...
                        {phoneKeys.length - 1 > 0 && (
                          <i
                            className="icon icon-trash gx-margin"
                            onClick={() =>
                              removeKey(index, 'phoneKeys', 'phone')
                            }
                          />
                        )}
                        {index == phoneKeys.length - 1 &&
                          phoneKeys.length <= 2 && (
                            <i
                              className="icon icon-add-circle"
                              onClick={() => addNewKey(index, 'phoneKeys')}
                            />
                          )}
                      </Form.Item>
                    );
                  ...
                  {addressKeys.map((k, index) => {
                    const req = index ? false : true;
                    const address = party?.address2?.details
                      ? [party?.address1?.details, party?.address2?.details]
                      : [party?.address1?.details];
                    const city = party?.address2?.city
                      ? [party?.address1?.city, party?.address2?.city]
                      : [party?.address1?.city];
                    const state = party?.address2?.state
                      ? [party?.address1?.state, party?.address2?.state]
                      : [party?.address1?.state];
                    const pincode = party?.address2?.pincode
                      ? [party?.address1?.pincode, party?.address2?.pincode]
                      : [party?.address1?.pincode];
                    return (
                      <div>
                        <Form.Item
                          {...formItemLayout}
                          className="address"
                          label={
                            index ? `Alternate address ${index}` : 'Address'
                          }
                        >
                          {getFieldDecorator(`address[${index}]`, {
                            ...
                            rules: [
                              {
                                required: req,
                                message: `Please input the ${index ? `Alternative address ${index}!` : 'Address!'}`,
                              },
                            ],
                          })(<Input id="address" />)}
                        </Form.Item>
                        ...
                        <div className="add-address">
                          {addressKeys.length - 1 > 0 && (
                            <i
                              className="icon icon-trash gx-margin"
                              onClick={() =>
                                removeKey(index, 'addressKeys', 'address')
                              }
                            />
                          )}
                          {index == addressKeys.length - 1 &&
                            addressKeys.length <= 1 && (
                              <i
                                className="icon icon-add-circle"
                                onClick={() => addNewKey(index, 'addressKeys')}
                              >
                                {' '}
                  ...
                  {emailKeys.map((k, index) => {
                    const req = index ? false : true;
                    const email = party?.email3
                      ? [party?.email1, party?.email2, party?.email3]
                      : party?.email2
                        ? [party?.email1, party?.email2]
                        : [party?.email1];
                    return (
                      <Form.Item
                        {...formItemLayout}
                      ...
                      >
                        {getFieldDecorator(`email[${index}]`, {
                          ...
                          rules: [
                            {
                              required: req,
                              message: `Please input your ${index ? `Alternate e-mail address ${index}!` : 'e-mail address!'}`,
                            },
                          ],
                        })(<Input id="email" />)}
                        {emailKeys.length - 1 > 0 && (
                          <i
                            className="icon icon-trash gx-margin"
                            onClick={() =>
                              removeKey(index, 'emailKeys', 'email')
                            }
                          />
                        )}
                        {index == emailKeys.length - 1 &&
                          emailKeys.length <= 2 && (
                            <i
                              className="icon icon-add-circle"
                              onClick={() => addNewKey(index, 'emailKeys')}
                            />
                          )}
                      </Form.Item>
                    );
                  ...
                  <Form.Item label="TAN Number">
                    {getFieldDecorator('tanNumber', {
                      rules: [
                        {
                          required: false,
                          message: 'Please input the TAN Number!',
                        },
                      ],
                    })(<Input id="tanNumber" />)}
                  </Form.Item>
                  ...
                  <Form.Item label="End User Tags">
                    {getFieldDecorator('endUsertags', {
                      rules: [
                        {
                          required: true,
                          message: 'Please enter End UserTags!',
                        },
                      ],
                    })(
                      <Select
                  ...
                  <Form.Item label="Daily Reports">
                    <Checkbox.Group
                      ...
                      onChange={checkedValues => {
                        setDailyReportsList(checkedValues);
                      }}
                    >
                      ...
                      <Checkbox value="STOCKSUMMARYREPORT">
                        STOCKSUMMARYREPORT
                      </Checkbox>
