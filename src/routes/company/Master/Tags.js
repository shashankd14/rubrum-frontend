...
const Tags = (props) => {
  ...
  const [tagsList, setTagsList] = useState(
    tabKey === "1" ? props?.processTags : props?.endUserTags
  );
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (text, record, index) => (
        <span>
          <span className="gx-link" onClick={(e) => onEdit(e, record)}>
            Edit
          </span>
          <Divider type="vertical" />
          <span className="gx-link" onClick={(e) => onDelete(e, record)}>
            Delete
          </span>
        </span>
      ),
    ...
    if (showEditTags) {
      ...
      props.form.validateFields((err, values) => {
        if (!err) {
          ...
          payload.push(values);
          tabKey === "1"
            ? props.addProccessTags(payload)
            : props.addEndUserTags(payload);
  ...
  return (
    <div>
      <h1>
        <IntlMessages id="sidebar.company.tagsList" />
      </h1>
                      ...
                      <Form.Item label="Tag Name">
                        {getFieldDecorator("tagName", {
                          rules: [
                            {
                              required: true,
                              message: "Please enter Tags name!",
                            },
                          ],
                        })(<Input id="tagName" />)}
                      </Form.Item>
