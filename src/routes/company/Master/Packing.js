...
const Packing = props => {
  ...
  const bucketColumns = [
    ...
    },
    {
      ...
      sorter: (a, b) =>
        a.packingBucketDesc?.length - b.packingBucketDesc?.length,
      sortOrder:
        sortedInfo.columnKey === 'packingBucketDesc' && sortedInfo.order,
    },
    {
      title: 'Action',
      dataIndex: '',
    ...
    if (searchValue) {
      if (tabKey === '1') {
        const filteredData = packing?.packingList?.filter(packing => {
          if (
            packing.packingItemId?.toString()?.includes(searchValue) ||
        ...
        const filteredData = packing?.bucketList?.filter(packing => {
          if (
            packing.packingBucketId?.toString()?.includes(searchValue) ||
  ...
  useEffect(() => {
    const units = props.packing?.bucket?.itemList
      ?.map(itm => itm.unit)
      .join(', ');
    setUnitValues(units);
  ...
  return (
    <div>
      <h1>
        <IntlMessages id="sidebar.company.packingList" />
      </h1>
      <Card>
        <div className="gx-flex-row gx-flex-1">
          ...
          <div className="gx-flex-row gx-w-50">
            ...
            >
              {tabKey === '1' ? 'Add Packing Item' : 'Add Packing Bucket'}
            </Button>
            <SearchBox
              styleName="gx-flex-1"
              placeholder="Search for packing or packing Item..."
              value={searchValue}
        ...
        >
          <Card>
            <Row>
              <Col span={24}>
                <Card>
                  {viewPackingData?.packingItemId && (
                    <p>
                      <strong>Packing Id :</strong>{' '}
                      {viewPackingData.packingItemId}
                    </p>
                  )}
                  {viewPackingData?.description && (
                    <p>
                      <strong>Packing description :</strong>{' '}
                      {viewPackingData?.description}
                    </p>
                  )}
                  {viewPackingData?.unit && (
                    <p>
                      <strong>Packing unit :</strong> {viewPackingData?.unit}
                    </p>
                  )}
                </Card>
              </Col>
        ...
        >
          <Card>
            <Row>
              <Col span={24}>
                <Card>
                  {viewBucketData?.packingBucketId && (
                    <p>
                      <strong>Bucket Id :</strong>{' '}
                      {viewBucketData?.packingBucketId}
                    </p>
                  )}
                  {viewBucketData?.packingBucketDesc && (
                    <p>
                      <strong>Bucket description :</strong>{' '}
                      {viewBucketData?.packingBucketDesc}
                    </p>
                  )}
                  {viewBucketData?.itemList && (
                    <p>
                      <strong>Packing Item list :</strong>{' '}
                      {viewBucketData?.itemList
                        ?.map(itm => itm.packingItemId)
                        .join(', ')}
                    </p>
                  )}
                  {viewBucketData?.itemList && (
                    <p>
                      <strong>Packing Item units :</strong>{' '}
                      {viewBucketData?.itemList
                        ?.map(itm => itm.unit)
                        .join(', ')}
                    </p>
                  ...
                  {viewBucketData?.qty && (
                    <p>
                      <strong>Qty :</strong> {viewBucketData?.qty}
                    </p>
                  )}
                </Card>
              </Col>
                ...
                <Form name="item" {...formItemLayout} className="gx-pt-4">
                  <Form.Item label="Packing Id">
                    {getFieldDecorator('packingId', {
                      rules: [
                        { required: false, message: 'Please enter packing Id' },
                      ],
                  ...
                  <Form.Item label="Packing unit">
                    {getFieldDecorator('unit', {
                      rules: [
                        {
                          required: true,
                          message: 'Please enter Packing unit',
                        },
                      ],
                ...
                <Form name="bucket" {...formItemLayout} className="gx-pt-4">
                      ...
                      <Select
                        ...
                        onChange={value => {
                          const unitVal = value?.map(itm => {
                            const obj = props?.packing?.packingList?.filter(
                              i => i.itemId === itm,
                            )[0];
                            return obj.unit;
                          });
                  ...
                  <Form.Item label="Packing bucket Id">
                    {getFieldDecorator('bucketId', {
                      rules: [
                        {
                          required: true,
                          message: 'Please enter packing bucket Id',
                        },
                      ],
                  ...
                  <Form.Item label="Units">
                    {getFieldDecorator('units', {
                      initialValue: props.packing?.bucket?.itemList
                        ?.map(itm => itm.unit)
                        .join(', '),
                      rules: [{ required: false }],
                    })(
                      <span id="unit" {...getFieldProps}>
                        {unitValues}
                      </span>,
                    )}
                  </Form.Item>
  ...
  mapPropsToFields(props) {
    return {
      ...
      units: Form.createFormField({
        value:
          props.packing?.bucket?.itemList?.map(itm => itm.unit).join(', ') ||
          '',
      }),
