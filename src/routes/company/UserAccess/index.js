...
const UserAccess = props => {
  ...
  const columns = [
    ...
    },
    {
      ...
      key: 'enabled',
      render: (_, record) => (
        <div>{record.enabled === 1 ? 'Active' : 'Disabled'}</div>
      ),
    },
    {
      title: 'Contact / Email',
  ...
  const getAllUsers = () => {
    axios
      .get(`${baseUrl}api/user/getAll`, { headers: getHeaders() })
      .then(response => {
  ...
  const fetchCompanyData = () => {
    axios
      .get(`${baseUrl}api/party/list`, { headers: getHeaders() })
      .then(response => {
  ...
  const fetchRoleDetails = () => {
    axios
      .get(`${baseUrl}api/role/getRoleDetails`, { headers: getHeaders() })
      .then(response => {
  ...
  }, [companyData.length, roleData.length]);
  ...
  const onDeleteUser = userId => {
    axios
      .delete(`${baseUrl}api/user/${userId}`, { headers: getHeaders() })
      .then(response => {
        ...
        <Form layout="horizontal" onSubmit={handleSubmit} form={props.form}>
          ...
          <FormItem {...formItemLayout} label="Full Name">
            {getFieldDecorator('fullName', {
              initialValue:
                userEditValues?.firstName || userEditValues?.lastName
                  ? `${userEditValues?.firstName} ${userEditValues?.lastName}`
                  : '',
              rules: [
          ...
          <FormItem {...formItemLayout} label="Company Name">
            {getFieldDecorator('companyName', {
              initialValue:
                userEditValues?.userPartyMap?.length > 0
                  ? userEditValues?.userPartyMap[0].partyId
                  : [],
              rules: [
              ...
              <Select mode="multiple" placeholder="Select a company">
                {companyData.length > 0 &&
                  companyData.map(company => (
                    <Option selected={true} value={company.value}>
                      {company.label}
                    </Option>
                  ))}
              </Select>,
          ...
          <FormItem {...formItemLayout} label="Role">
            {getFieldDecorator('role', {
              initialValue:
                userEditValues?.userRoleMap?.length > 0
                  ? userEditValues?.userRoleMap[0].roleId
                  : [],
