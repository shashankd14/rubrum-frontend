    ...
    useEffect(() => {
        ...
        if(data !== undefined){
            if(props.childCoil){
                ...
                data = data.flat();
                let cutsData = [...data];
                cutsData = props.wip ? (props.slitCut ? cutsData.filter(item => item.process.processId === 3 && item.status.statusId !==3 && item.groupId !== null) : cutsData.filter(item => item.process.processId === 1 && item.status.statusId !==3)) : cutsData.filter(item => item.process.processId === 1)
                setCuts(cutsData);
            }
        }
       }}, [props.coilDetails]);
