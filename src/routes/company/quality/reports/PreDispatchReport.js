...
const PreDispatchReport = props => {
  ...
  const columns = [
    ...
    },
    {
      ...
      sortOrder:
        sortedInfo.columnKey === 'deliveryChalanNo' && sortedInfo.order,
      onCell: record => ({
        className: 'gx-link',
    ...
    },
    {
      ...
      sortOrder: sortedInfo.columnKey === 'endUserTags' && sortedInfo.order,
    },
    {
      ...
      sortOrder:
        sortedInfo.columnKey === 'customerInvoiceNo' && sortedInfo.order,
    },
    {
      ...
      sortOrder:
        sortedInfo.columnKey === 'customerInvoiceDate' && sortedInfo.order,
    },
    {
      title: 'Status',
      dataIndex: 'status',
  ...
  }, [customerValue, props, searchValue]);
  ...
  useEffect(() => {
    if (!isInitialMount.current) {
      if (
        ...
        props.history.push({
          pathname: '/company/quality/reports/create/predispatch',
          state: {
        ...
        setTemplateLinkList(
          tempData.filter(x => x.stageName === 'PRE_DISPATCH'),
        );
  ...
  }, [props.inward.inwardList, props.inward.loading, props.inward.success]);
  ...
  }, [props.party.loading, props.party.error, props.party]);

  useEffect(() => {
    const { template } = props;
    if (searchValue) {
      const filteredData = filteredPreDispatchList.filter(
        item =>
  ...
  }, [filteredPreDispatchList, props, searchValue]);
              ...
              >
                {templateLinkList.length > 0 &&
                  templateLinkList.map(link => (
                    ...
                    >{`${link.templateId}-${link.templateName}`}</Select.Option>
                  ))}
              </Select>
            </Row>
          </Col>
