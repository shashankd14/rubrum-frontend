...
const CreateTemplate = props => {
  ...
  useEffect(() => {
    console.log(props);
    if (
      !props.templateDetails.loading &&
      !props.templateDetails.error &&
      props.templateDetails.operation === 'templateById'
    ) {
  ...
  const handleCreate = data => {
    ...
    if (action == 'create') props.saveQualityTemplate(request);
    else if (action == 'edit') props.updateQualityTemplate(request);
      ...
      <Card>
        <div className="gx-flex-row gx-flex-1">
          <div className="table-operations gx-col">
            ...
            />
            {templateNameErr && (
              <p style={{ color: 'red' }}>Please enter Template Name</p>
            )}
          </div>
