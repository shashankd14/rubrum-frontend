...
const CreateLinkTemplate = props => {
  ...
  useEffect(() => {
    if (
      !props.template.loading &&
      !props.template.error &&
      props.template.operation === 'templateLinkById'
    ) {
      const urlPaths = props.match.url.split('/');
      setSelectedTemplateId(urlPaths[urlPaths.length - 1]);
  ...
  const setSelectedTemplateDetails = templateId => {
    ...
    setDefaultSelected(
      filteredTemplate?.length >= 1 ? [filteredTemplate[0].templateId] : [],
    );
  };
    ...
    if (e.length > 2) {
      setTemplateList(
        data.filter(
          t =>
            t.templateId == e ||
            t.templateName.toLowerCase().includes(e.toLowerCase()),
        ),
      );
    } else {
  ...
  useEffect(() => {
    if (
      !props.template.loading &&
      !props.template.error &&
      props.template.operation === 'templateLinkSave'
    ) {
  ...
  const createTemplateLink = () => {
    ...
    if (action == 'create') props.saveQualityTemplateLink(payload);
    else if (action == 'edit') {
  ...
  const selectAllMaterialGrades = () => {
    ...
    console.log(
      'mat.materialGrade?.gradeId',
      props.material?.materialList.gradeId,
    );
  ...
  return (
    <div>
      <Card title="Link Template">
        ...
        <Row>
          <Col span={12}>
            <label style={{ marginTop: 5, display: 'flex' }}>
              Stage: {selectedTemplate?.stageName}
            </label>
          </Col>
        </Row>
        <Row>
                ...
                <div style={{ marginTop: 30, display: 'flex' }}>
                  ...
                  <Checkbox
                    ...
                    checked={
                      ...
                      props.template.thicknessList.length
                    }
                    ...
                    filterOption={
                      (input, option) =>
                        option.props.value
                          .toString()
                          .toLowerCase()
                          .indexOf(input.toLowerCase()) >= 0 // Use option's value for filtering
                    }
                    allowClear
                  >
