...
const LabelPrintWIP = (props) => {
  ...
  const onPdf = (inwardEntryId) => {
    ...
    const payloadpdf = { inwardEntryId: inwardEntryId, process: 'WIP' };
    props.labelPrintWIP(payloadpdf);
  };

  const getsPlanLabel = () => {
    return (
      <>
        {props.labelPrint?.wipLabelPdf?.wip_labels?.length > 0 && (
          <div>
            <p>WIP Labels</p>
    ...
    <>
      <div className="gx-flex-row gx-flex-1">
        ...
        <div className="table-operations gx-col">
          <SearchBox
            styleName="gx-flex-1"
            placeholder="Search by Batch no. or SC inward id"
            value={searchValue}
