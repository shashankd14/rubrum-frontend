...
function* fetchMaterialList() {
  try {
    ...
    } else yield put(fetchMaterialListError('error'));
  } catch (error) {
...
function* addMaterial(action) {
  try {
    ...
    if (addMaterial.status === 200) {
      ...
      yield put(userSignOutSuccess());
    } else yield put(addMaterialError('error'));
  } catch (error) {
...
function* updateMaterial(action) {
  try {
    const {
      values: { description, grade, hsnCode, materialCode },
      id,
    } = action.material;
    const materialObj = {
      matId: id,
    ...
    if (updateMaterial.status === 200) {
      ...
      yield put(userSignOutSuccess());
    } else yield put(updateMaterialError('error'));
  } catch (error) {
...
function* fetchMaterialListById(action) {
  try {
    const fetchMaterialById = yield fetch(
      `${baseUrl}api/material/getById/${action.materialId}`,
      {
        method: 'GET',
        headers: getHeaders(),
      },
    ...
    if (fetchMaterialById.status === 200) {
      ...
      yield put(userSignOutSuccess());
    } else yield put(fetchMaterialListByIdError('error'));
  } catch (error) {
