...
function* fetchPartyList() {
  try {
    ...
    } else yield put(fetchPartyListError('error'));
  } catch (error) {
...
function* fetchPartyListById(action) {
  try {
    const fetchPartyListId = yield fetch(
      `${baseUrl}api/party/getById/${action.partyId}`,
      {
        method: 'GET',
        headers: getHeaders(),
      },
    ...
    if (fetchPartyListId.status === 200) {
      ...
      yield put(userSignOutSuccess());
    } else yield put(fetchPartyListIdError('error'));
  } catch (error) {
...
function* addParty(action) {
  try {
    ...
    if (addParty.status == 200) {
      ...
      yield put(userSignOutSuccess());
    } else yield put(addPartyError('error'));
  } catch (error) {
...
function* updateParty(action) {
  try {
    ...
    if (updateParty.status == 200) {
      ...
      yield put(userSignOutSuccess());
    } else yield put(updatePartyError('error'));
  } catch (error) {
    console.log(error);
