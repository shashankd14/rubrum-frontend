...
function* fetchPackingList() {
  try {
    ...
    } else yield put(fetchPackingListError('error'));
  } catch (error) {
...
function* fetchPackingListByParty(action) {
  try {
    const fetchPackingListByParty = yield fetch(
      `${baseUrl}api/packing/rate/party/${action?.partyId[0]}`,
      {
        method: 'GET',
        headers: getHeaders(),
      },
    ...
    if (fetchPackingListByParty.status === 200) {
      const fetchPackingListByPartyResponse =
        yield fetchPackingListByParty.json();
      yield put(
        fetchPackingListByPartySuccess(fetchPackingListByPartyResponse),
      );
    } else if (fetchPackingListByParty.status === 401) {
      yield put(userSignOutSuccess());
    } else yield put(fetchPackingListByPartyError('error'));
  } catch (error) {
...
function* fetchPackingBucketList() {
  try {
    ...
    if (fetchPackingBucketList.status === 200) {
      const fetchPackingBucketListResponse =
        yield fetchPackingBucketList.json();
      yield put(fetchPackingBucketListSuccess(fetchPackingBucketListResponse));
    } else if (fetchPackingBucketList.status === 401) {
      yield put(userSignOutSuccess());
    } else yield put(fetchPackingBucketListError('error'));
  } catch (error) {
...
function* addPacking(action) {
  try {
    ...
    if (addPacking.status == 200) {
      ...
      yield put(userSignOutSuccess());
    } else yield put(addPackingError('error'));
  } catch (error) {
...
function* addPackingBucket(action) {
  try {
    ...
    if (addPackingBucket.status == 200) {
      ...
      yield put(userSignOutSuccess());
    } else yield put(addPackingBucketError('error'));
  } catch (error) {
...
function* updatePacking(action) {
  try {
    const {
      values: { packingId: packingItemId, description, unit },
      id,
    } = action.packing;
    const packingObj = {
      id,
    ...
    if (updatePacking.status == 200) {
      ...
      yield put(userSignOutSuccess());
    } else yield put(updatePackingError('error'));
  } catch (error) {
...
function* updatePackingBucket(action) {
  try {
    const {
      values: { bucketDesc, bucketId, packingItem, qty },
      id,
    } = action.bucket;
    const packingObj = {
      bucketId: id,
    ...
    const updatePackingBucket = yield fetch(
      `${baseUrl}api/packing/bucket/update`,
      {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json', ...getHeaders() },
        body: JSON.stringify(packingObj),
    ...
    if (updatePackingBucket.status == 200) {
      ...
      yield put(userSignOutSuccess());
    } else yield put(updatePackingBucketError('error'));
  } catch (error) {
...
function* fetchPackingListById(action) {
  try {
    const fetchPackingById = yield fetch(
      `${baseUrl}api/packing/item/${action.packingId}`,
      {
        method: 'GET',
        headers: getHeaders(),
      },
    ...
    if (fetchPackingById.status === 200) {
      ...
      yield put(userSignOutSuccess());
    } else yield put(fetchPackingListByIdError('error'));
  } catch (error) {
...
function* fetchPackingBucketListById(action) {
  try {
    const fetchPackingBucketListById = yield fetch(
      `${baseUrl}api/packing/bucket/${action.bucketId}`,
      {
        method: 'GET',
        headers: getHeaders(),
      },
    ...
    if (fetchPackingBucketListById.status === 200) {
      ...
      yield put(userSignOutSuccess());
    } else yield put(fetchPackingBucketListByIdError('error'));
  } catch (error) {
