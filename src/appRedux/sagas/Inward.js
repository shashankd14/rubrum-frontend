...
function* fetchInwardListWithOldAPI({
  page = 1,
  pageSize = 15,
  searchValue = '',
  partyId = '',
  ...
  try {
    const fetchInwardList = yield fetch(
      `${baseUrl}api/inwardEntry/list/${page}/${pageSize}?searchText=${searchValue}&partyId=${partyId}`,
      {
        method: 'GET',
        headers: getHeaders(),
      },
  ...
  try {
    ...
    data.append(
      ...
      action.inward.grade !== undefined
        ? action.inward.grade
        : Number(action.inward.materialGrade.gradeId),
    );
